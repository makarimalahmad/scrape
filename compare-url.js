...
function parseProduct(name) {
  ...
  const namedProducts = [
    ...
    [/monthly epic (?:pack|bundle)/, 'Monthly Epic Pack', 'pass'],
  ...
  for (const [pattern, key, category] of namedProducts) {
    if (pattern.test(clean)) return { key, category, totalDiamonds: null };
  }

  const leadingTotalMatch = clean.match(/^(\d[\d.,]*)\s*\(\s*\d[\d.,]*\s*\+\s*\d[\d.,]*\s*\)\s*(?:diamonds?|diaomonds?|dm)/i);
  const bonusMatch = clean.match(/(\d[\d.,]*)\s*\+\s*(\d[\d.,]*)\s*(?:diamonds?|diaomonds?|dm)/i);
  const diamondMatch = clean.match(/(\d[\d.,]*)\s*(?:diamonds?|diaomonds?|dm)/i);

  let totalDiamonds = null;
  if (leadingTotalMatch) {
    totalDiamonds = parseCount(leadingTotalMatch[1]);
  } else if (bonusMatch) {
    totalDiamonds = parseCount(bonusMatch[1]) + parseCount(bonusMatch[2]);
  } else if (diamondMatch) {
    totalDiamonds = parseCount(diamondMatch[1]);
  }

  if (totalDiamonds !== null) {
    let category = 'diamond';
    if (/first top[ -]?up|pembelian pertama/.test(clean)) category = 'diamond-first-topup';
    if (/vision|wetv|duolingo|prime video|subscription/.test(clean)) category = 'diamond-service-bonus';
    return { key: `${totalDiamonds} Diamonds`, category, totalDiamonds };
  }

  const key = clean.replace(/\b(?:from|dari|promo|diskon)\b/gi, '').replace(/\s+/g, ' ').trim();
  return { key, category: 'other', totalDiamonds: null };
}
...
function cheapestProducts(rows) {
  const map = new Map();

  for (const row of rows) {
    const parsed = parseProduct(row.Produk);
    ...
    if (!existing || price < existing.price) {
      map.set(mapKey, {
        ...
        mapKey,
        price,
        pricePerDiamond: parsed.totalDiamonds ? price / parsed.totalDiamonds : null,
        rawName: row.Produk,
        rawPrice: row.Harga
      });
    }
...
function findMatches(mainProduct, products) {
  const exact = products.get(mainProduct.mapKey);
  if (exact) return [{ product: exact, type: 'Exact', diamondDifference: 0 }];
  if (mainProduct.category !== 'diamond' || mainProduct.totalDiamonds === null) return [];

  const maxDifference = Math.min(5, Math.max(1, Math.ceil(mainProduct.totalDiamonds * 0.1)));
  return Array.from(products.values())
    .filter((candidate) =>
      candidate.category === 'diamond' &&
      candidate.totalDiamonds !== null &&
      Math.abs(candidate.totalDiamonds - mainProduct.totalDiamonds) <= maxDifference
    )
    .sort((a, b) => {
      const distanceA = Math.abs(a.totalDiamonds - mainProduct.totalDiamonds);
      const distanceB = Math.abs(b.totalDiamonds - mainProduct.totalDiamonds);
      return distanceA - distanceB || a.pricePerDiamond - b.pricePerDiamond;
    })
    .slice(0, 3)
    .map((product) => ({
      product,
      type: 'Proximity',
      diamondDifference: product.totalDiamonds - mainProduct.totalDiamonds
    }));
}
...
async function scrapeUrl(rawUrl, headed) {
  ...
  return { url: url.href, store: getStore(url.href), products: cheapestProducts(rows) };
}
