...
async function searchGoogle(
  ...
  // Auto-pagination: jika kuota kompetitor belum terpenuhi di Halaman 1, ambil Halaman 2 (start=10)
  if (ranking.length < limit) {
    parameters.set("start", "10");
    try {
      const page2Response = await fetchFunction(
        `https://serpapi.com/search.json?${parameters}`,
      );
      if (page2Response.ok) {
        const page2Data = await page2Response.json();
        if (!page2Data.error && Array.isArray(page2Data.organic_results)) {
          organicResults.push(...page2Data.organic_results);
