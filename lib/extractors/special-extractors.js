...
async function extractDuniaGamesRows(page) {
  const ready = await page
    .waitForFunction(
      () => {
        const hasErrorPopup =
          document.querySelector("app-popup-error-transaction, .error-payment-wrapper, [class*='error-payment']") ||
          Array.from(
            document.querySelectorAll("[role='dialog'], .cdk-overlay-pane, [class*='modal'], [class*='popup']"),
          ).some((el) => /item habis|stok habis|pemeliharaan|maintenance/i.test(el.innerText || ""));
        if (hasErrorPopup) return true;
...
async function extractMobapayRows(page) {
  ...
  return page.evaluate(() => {
    ...
    const mainWrapper =
      wrappers.find((w) => w.querySelector(".mobapay-scroll-recharge-arrow")) ||
      wrappers[1] ||
      wrappers[0];
    if (!mainWrapper) return [];
    const items = Array.from(mainWrapper.querySelectorAll(".mobapay-recharge-item"));
  ...
  return page.evaluate(() => {
    ...
    for (const card of leafCards) {
      ...
      const name =
        lines.find(
          (l) =>
            !/^(?:Dari|Promo|Diskon|\+[\d%]+ Bonus|Termurah|\d+%)$/i.test(l) &&
            !/^Rp[\d.]+/i.test(l),
        ) || lines[0];

      const priceLines = lines.filter((l) => /^Rp[\d.]+/i.test(l) || /^Dari\s*Rp[\d.]+/i.test(l));
      if (!name || !priceLines.length) continue;
      ...
      if (!seen.has(key)) {
        seen.add(key);
        rows.push({ Produk: name, Harga: price });
      }
    }
    return rows;
  });
...
async function extractSpecialRows(page, url, interceptedPayloads = []) {
  ...
  if (hostname === "mobapay.com") return extractMobapayRows(page);
  if (hostname === "lapakgaming.com") return extractLapakgamingRows(page);
  if (hostname === "unipin.com" && /roblox/i.test(url.pathname)) {
