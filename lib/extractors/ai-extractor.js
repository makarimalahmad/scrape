...
/**
 * Mengekstrak dan mem-parse JSON dari teks respons AI.
 * Menangani format JSON murni, blok markdown ```json ... ```, maupun teks dengan pembungkus.
 */
function extractJsonFromContent(rawContent) {
  ...
  try {
    ...
    if (firstBrace !== -1 && lastBrace !== -1 && lastBrace > firstBrace) {
      try {
        return JSON.parse(unquoted.slice(firstBrace, lastBrace + 1));
      } catch {}
    }
    return null;
  }
...
${cleanedText}`;
  ...
  try {
    ...
    const trimmedRaw = rawResponseText.trim();
    if (trimmedRaw.startsWith("data:")) {
      // Format SSE streaming (Server-Sent Events) dari router/proxy seperti 9router
    ...
    if (!parsed) {
      console.log(
        "[AI Fallback] Gagal membaca JSON dari respons AI:",
        content.slice(0, 150),
      );
      return null;
    }
