...
async function searchGoogle(
  ...
  // Smart fallback: jika hasil ranking kompetitor terlalu sedikit (< Math.min(limit, 5)) dan belum menggunakan filter "0",
  // coba query ulang menggunakan filter "0" agar Google membuka toko-toko yang disaring sebagai hasil serupa
  const minExpected = Math.min(limit, 5);
  if (ranking.length < minExpected && !parameters.has("filter")) {
    console.log(
      `[Google Search] Toko kompetitor organik sedikit (${ranking.length} < ${minExpected}). Mencoba ulang dengan filter: "0"...`,
    );
    parameters.set("filter", "0");
    try {
