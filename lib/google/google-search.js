...
async function fetchBrightDataSerp(apiKey, query, options = {}) {
  ...
  const hasSerpApiBackup = Boolean(process.env.SERPAPI_KEY);
  const effectiveMaxAttempts = hasSerpApiBackup ? Math.min(2, maxAttempts) : maxAttempts;
  ...
  for (let attempt = 1; attempt <= effectiveMaxAttempts; attempt += 1) {
    try {
      ...
      if (!rawText || statusCode !== "200") {
        ...
        const isRejectedRedirect = /redirect location was rejected/i.test(errDetail);

        // Jika ada cadangan SerpApi dan error adalah cooldown/redirect, fail-fast agar langsung dioper ke SerpApi
        if (hasSerpApiBackup && (isCooldown || isRejectedRedirect)) {
          throw lastError;
        }
      ...
      const isRejectedRedirect = /redirect location was rejected/i.test(error.message);
...
async function searchGoogle(
  ...
  const brightDataKey = process.env.BRIGHTDATA_API_KEY || (provider === "brightdata" ? apiKey : null);
  const serpApiKey = process.env.SERPAPI_KEY || (provider === "serpapi" ? apiKey : null);

  if (provider === "brightdata" && !brightDataKey && serpApiKey) {
    provider = "serpapi";
  ...
  if (provider === "brightdata") {
    if (!brightDataKey) {
      if (serpApiKey) {
        console.warn("[Bright Data] Kunci API belum diatur, beralih ke SerpApi...");
        provider = "serpapi";
        process.env.SERP_PROVIDER = "serpapi";
        organicResults = await fetchSerpApiSerp(serpApiKey, gameConfig.query, {
          start: 0,
          searchDepth,
        ...
        throw new Error("BRIGHTDATA_API_KEY belum diatur.");
      }
    } else {
      try {
        ...
        if (serpApiKey) {
          ...
          provider = "serpapi";
          process.env.SERP_PROVIDER = "serpapi"; // Kunci untuk game & query selanjutnya di sesi ini
          organicResults = await fetchSerpApiSerp(serpApiKey, gameConfig.query, {
            start: 0,
            searchDepth,
            serpapiFilter: gameConfig.serpapiFilter,
    ...
    try {
      organicResults = await fetchSerpApiSerp(serpApiKey || apiKey, gameConfig.query, {
        start: 0,
        searchDepth,
        serpapiFilter: gameConfig.serpapiFilter,
        fetchFunction: resolvedFetch,
      ...
      if (brightDataKey) {
        console.warn(`[SerpApi] Gagal (${serpErr.message}). Beralih otomatis ke cadangan Bright Data...`);
        provider = "brightdata";
        process.env.SERP_PROVIDER = "brightdata"; // Kunci untuk game & query selanjutnya di sesi ini
        organicResults = await fetchBrightDataSerp(brightDataKey, gameConfig.query, {
          start: 0,
          fetchFunction: resolvedFetch,
        });
    ...
    try {
      ...
      if (provider === "brightdata") {
        try {
          ...
          if (serpApiKey) {
            console.warn(`[Bright Data Pagination] Gagal (${brightNextErr.message}). Beralih otomatis ke SerpApi...`);
            provider = "serpapi";
            process.env.SERP_PROVIDER = "serpapi";
            nextPageResults = await fetchSerpApiSerp(serpApiKey, gameConfig.query, {
              start: startOffset,
              searchDepth,
              serpapiFilter: gameConfig.serpapiFilter,
              fetchFunction: resolvedFetch,
        ...
        try {
          nextPageResults = await fetchSerpApiSerp(serpApiKey || apiKey, gameConfig.query, {
            start: startOffset,
            searchDepth,
            serpapiFilter: gameConfig.serpapiFilter,
            fetchFunction: resolvedFetch,
          ...
          if (brightDataKey) {
            console.warn(`[SerpApi Pagination] Gagal (${serpNextErr.message}). Beralih otomatis ke Bright Data...`);
            provider = "brightdata";
    ...
    try {
      ...
      if (provider === "brightdata") {
        try {
          ...
          if (serpApiKey) {
            console.warn(`[Bright Data Alt Query] Gagal (${brightAltErr.message}). Beralih otomatis ke SerpApi...`);
            provider = "serpapi";
            process.env.SERP_PROVIDER = "serpapi";
            fallbackOrganic = await fetchSerpApiSerp(serpApiKey, altQuery, {
              start: 0,
              searchDepth,
              fetchFunction: resolvedFetch,
            });
        ...
        try {
          fallbackOrganic = await fetchSerpApiSerp(serpApiKey || apiKey, altQuery, {
            start: 0,
            searchDepth,
            fetchFunction: resolvedFetch,
          });
          ...
          if (brightDataKey) {
            console.warn(`[SerpApi Alt Query] Gagal (${serpAltErr.message}). Beralih otomatis ke Bright Data...`);
            provider = "brightdata";
            process.env.SERP_PROVIDER = "brightdata";
            fallbackOrganic = await fetchBrightDataSerp(brightDataKey, altQuery, {
              start: 0,
