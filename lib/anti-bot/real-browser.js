...
function getRealBrowserDomains() {
  const raw = process.env.REAL_BROWSER_DOMAINS || "";
  return raw
    .split(",")
...
const REAL_BROWSER_DOMAINS = getRealBrowserDomains();
  ...
  try {
    ...
    for (let s = 1; s <= 35; s += 1) {
      ...
      if (!isBlockedOrChallenge) {
        console.log(`[Real Browser] Berhasil melewati verifikasi Cloudflare (${s}s) - ${title}`);
        passed = true;
        break;
      }
    }
    ...
    console.log(`[Real Browser] Selesai mengekstrak ${finalRows.length} produk untuk ${domain}.`);
    return finalRows;
  } finally {
