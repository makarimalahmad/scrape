...
function classifyTopUpCompetitorResult(result, gameConfig) {
  ...
  const isEditorialSubdomain = /^(?:news|blog|blogs|artikel|article)\./i.test(hostname);
  const editorialPath = /\/(?:blog|blogs|artikel|article|articles|news|berita|guide|panduan|tips?|tag|tags)(?:\/|$)|\/(?:cara|how-to)-/i.test(
    pathname,
  );
  const editorialTitle = /^(?:\d+\s+)?(?:cara|tips?|panduan|tutorial|rekomendasi|daftar)\b|\byang perlu diketahui\b/i.test(
    title,
  );
  if (isEditorialSubdomain || editorialPath || editorialTitle) {
...
async function searchGoogle(
  ...
  if (gameConfig.serpapiFilter != null) {
    parameters.set("filter", gameConfig.serpapiFilter);
  }
  ...
  // Auto-pagination: jika kuota kompetitor belum terpenuhi di Halaman 1, ambil halaman berikutnya (start=10, 20)
  let startOffset = 10;
  while (ranking.length < limit && startOffset <= 30) {
    parameters.set("start", String(startOffset));
    try {
      const nextPageResponse = await fetchFunction(
        `https://serpapi.com/search.json?${parameters}`,
      );
      if (!nextPageResponse.ok) break;
      const nextPageData = await nextPageResponse.json();
      if (nextPageData.error || !Array.isArray(nextPageData.organic_results) || !nextPageData.organic_results.length) {
        break;
      }
      organicResults.push(...nextPageData.organic_results);
  ...
  // Smart fallback: jika hasil ranking kompetitor terlalu sedikit (< Math.min(limit, 5)),
  // coba query alternatif komersial lokal agar Google memberikan daftar toko yang lebih lengkap tanpa spam
  ...
  if (ranking.length < minExpected && altQuery && altQuery !== gameConfig.query) {
    ...
    parameters.set("q", altQuery);
    try {
      const fallbackResponse = await fetchFunction(
        `https://serpapi.com/search.json?${parameters}`,
      );
      if (fallbackResponse.ok) {
        const fallbackData = await fallbackResponse.json();
        if (!fallbackData.error && Array.isArray(fallbackData.organic_results)) {
          const fallbackSelection = selectGoogleCompetitors(
            fallbackData.organic_results,
            gameConfig,
            limit,
          );
          if (fallbackSelection.ranking.length > ranking.length) {
            ...
            organicResults = fallbackData.organic_results;
          }
        }
      }
    } catch {}
...
async function scrapeWithRetry(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    try {
      ...
      if (error.proxyFailed || error.message.includes("[Proxy Error]")) {
        activeOptions = { ...activeOptions, disableProxy: true, forceProxy: false };
        const host = (url instanceof URL ? url.hostname : new URL(url).hostname).replace(/^www\./, "");
        console.log(`[Proxy Failover] Mencoba ulang ${host} menggunakan koneksi langsung (tanpa proxy)...`);
      }
...
async function scrapeStore(store, gameConfig, options) {
  ...
  for (const rawUrl of targetUrls) {
    ...
    allRows.push(...rows);
    if (rows._usedAiFallback) allRows._usedAiFallback = true;
