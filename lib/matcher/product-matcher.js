...
function parseRobloxProduct(name) {
  const robuxMatch =
    name.match(/(\d[\d.,]*)\s*(?:robux|rbx|r\$)\b/i) ||
    name.match(/(?:robux|rbx|r\$)\s*(\d[\d.,]*)\b/i);
  if (robuxMatch) {
  ...
  const sarMatch =
    name.match(/roblox(?:\s+gift\s+card)?\s*(\d[\d.,]*)\s*sar/i) ||
    name.match(/(\d[\d.,]*)\s*sar/i);
  if (sarMatch) {
    const quantity = parseCount(sarMatch[1]);
    return {
      key: `Roblox SAR ${quantity}`,
      category: "roblox-sar-card",
      quantity,
      unit: "SAR Voucher",
    };
  }
  ...
  const idrMatch =
    ...
    name.match(/roblox\s+gift\s+card\s+(\d[\d.,]*)(?!\s*(?:sar|usd))/i) ||
  ...
  const usdMatch =
    name.match(/(?:roblox\s*)?\(?\s*usd\s*\)?\s*\$?\s*(\d[\d.,]*)/i) ||
...
function parseDiamondProduct(name) {
  ...
  let suffix = "";
  if (/first top[ -]?up|pembelian pertama|pengisian pertama/i.test(name)) {
    category = "diamond-first-topup";
    suffix = " (First Top-up)";
  } else if (/vision|wetv|duolingo|prime video|subscription/i.test(name)) {
    category = "diamond-service-bonus";
    suffix = " (Service Bonus)";
  }

  return {
    key: `${quantity} Diamonds${suffix}`,
    category,
    quantity,
    unit: "Diamond",
  };
...
function extractDomain(hostname, url, store) {
  let host = hostname;
  if (!host && url) {
    try {
...
function applyTaxCalculation(calculateTax, payload) {
  if (
    !calculateTax ||
    typeof calculateTax !== "object" ||
    typeof payload !== "object" ||
    payload === null
  ) {
    return payload?.rawPrice ?? 0;
  }

  const rawPrice = payload.rawPrice ?? 0;
  const domainKey = String(payload.domain || "").toLowerCase();
  const hostKey = String(payload.hostname || "").toLowerCase();
  const storeKey = String(payload.store || "").toLowerCase();
  const cleanDomain = domainKey.replace(/^www\./, "");
  const cleanHost = hostKey.replace(/^www\./, "");

  let rule =
    calculateTax[domainKey] ??
    calculateTax[hostKey] ??
    calculateTax[storeKey] ??
    calculateTax[cleanDomain] ??
    calculateTax[cleanHost] ??
    calculateTax[`www.${cleanDomain}`];

  if (rule && typeof rule === "object" && !Array.isArray(rule)) {
    const gameRaw = String(payload.game || "").toLowerCase().trim();
    const gameNormalized = normalizeGame(gameRaw);
    rule = rule[gameRaw] ?? rule[gameNormalized] ?? rule["default"];
  }

  if (typeof rule === "function") {
    try {
      const adjusted = rule(rawPrice, payload);
      if (
    ...
    const cleaned = rule.replace(/%/g, "").trim();
    const percent = Number.parseFloat(cleaned);
    if (!Number.isNaN(percent) && percent >= 0) {
      return Math.round(rawPrice * (1 + percent / 100));
    }
  } else if (typeof rule === "number" && !Number.isNaN(rule) && rule >= 0) {
    if (rule === 0) return rawPrice;
    if (rule > 2) {
      return Math.round(rawPrice * (1 + rule / 100));
    }
    if (rule > 0 && rule < 1) {
      return Math.round(rawPrice * (1 + rule / 100));
    }
    return Math.round(rawPrice * rule);
  }
...
function selectCheapestProducts(rows, game, options = {}) {
  ...
  const store = options?.store || options?.name || "";
  const domain = extractDomain(hostname, url, store);
  ...
  for (const row of rows) {
    ...
    // Sanity check: abaikan mata uang asing (USD, EUR, dll.) atau harga yang tidak realistis (< Rp 500 untuk MLBB/FF)
    if (["mobile-legends", "free-fire"].includes(normalizeGame(game))) {
      if (/US\$|USD|\$|EUR|€|GBP|£|TRY|TL|RM|SGD/i.test(String(row.Harga || ""))) continue;
      if (price < 500) continue;
    }

    if (calculateTax) {
      price = applyTaxCalculation(calculateTax, {
        rawPrice: price,
...
function findMatches(mainProduct, competitorProducts) {
  ...
  // Langkah 2: Batas minimal toleransi adalah 1 Diamond
  if (maxDifference < 1) {
    maxDifference = 1;
  }

  // Langkah 3: Batas maksimal toleransi di 5 Diamond 
  if (maxDifference > 5) {
    maxDifference = 5;
  }
